...
class MetaTileDisplay extends StatelessWidget {
  ...
  const MetaTileDisplay({required this.tileData, this.showGrid = true, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) => DotMatrix(
      pixels: tileData.map((e) => context.read<AppStateCubit>().state.colorSet[e]).toList(),
      showGrid: showGrid,
